...
////MC
	...
	///OUTGOING
		...
		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_hist_monitoring_table where trunc(tran_date)=?1 and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListTotTxs(String tranDate);
		
		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_hist_monitoring_table where trunc(tran_date)=?1 and tran_status='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCLisSucTxs(String tranDate);

		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_hist_monitoring_table where trunc(tran_date)=?1 and tran_status!='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListFailTxs(String tranDate);
		
		@Query(value = "select nvl(sum(tran_amount),0) from bips_transaction_hist_monitoring_table where trunc(tran_date)=?1 and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListTotAmt(String tranDate);
		
		@Query(value = "select nvl(sum(tran_amount),0) from bips_transaction_hist_monitoring_table where trunc(tran_date)=?1 and tran_status='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
