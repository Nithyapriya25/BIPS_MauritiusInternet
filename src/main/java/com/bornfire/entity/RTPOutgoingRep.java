...
public interface RTPOutgoingRep extends JpaRepository<RTP_Outgoing_entity, String> {
	...
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE where (msg_type='OUTGOING') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
	List<RTP_Outgoing_entity> findAllCustomoutward(String tranDate);
	
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE where (msg_type='OUTWARD_BULK_RTP') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
	List<RTP_Outgoing_entity> findAllCustomoutwardrtp(String tranDate);
	
	@Query(value = "select cim_account,cim_account_name,ipsx_account,ipsx_account_name,tran_date,value_date,sequence_unique_id,tran_audit_number,msg_type,tran_amount,entry_user,master_ref_id,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=cdtr_agt)cdtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=dbtr_agt)dbtr_agt,init_channel_id,tran_currency,tran_status,cbs_status,cbs_status_error,response_status,ipsx_status_error,resv_field1,tran_type_code from BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE where (msg_type='OUTWARD_BULK_RTP') and  trunc(tran_date)=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
...
////RTP Outward
	...
	@Query(value = "select * from BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE where tran_date>=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
	...
	@Query(value="SELECT COUNT(CASE WHEN msg_type = 'OUTGOING' AND TRAN_STATUS ='SUCCESS' THEN 0 END) as outTranS,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTGOING' AND TRAN_STATUS ='FAILURE' THEN 0 END) as outTranF,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTGOING' AND TRAN_STATUS ='IN_PROGRESS' THEN 0 END) as outTranIP,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTGOING' AND TRAN_STATUS ='REVERSE_FAILURE' THEN 0 END) as outTranRF,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTWARD_BULK_RTP' AND TRAN_STATUS ='SUCCESS' THEN 0 END) as outRTPS,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTWARD_BULK_RTP' AND TRAN_STATUS ='FAILURE' THEN 0 END) as outRTPF,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTWARD_BULK_RTP' AND TRAN_STATUS ='IN_PROGRESS' THEN 0 END) as outRTPIP,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTWARD_BULK_RTP' AND TRAN_STATUS ='REVERSE_FAILURE' THEN 0 END) as outRTPRF,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTGOING'  THEN 0 END) as outTranT,\n" + 
			"COUNT(CASE WHEN msg_type = 'OUTWARD_BULK_RTP'  THEN 0 END) as outRTPT\n" + 
			"FROM BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE",nativeQuery=true)
	List<Object[]> findDashboardData();


	@Query(value="SELECT * FROM BIPS_OUTWARD_TRANSACTION_MONITORING_TABLE offset ? row fetch first ? rows only", nativeQuery = true)
	List<RTP_Outgoing_entity> findUserWithPage(int start, int length);
