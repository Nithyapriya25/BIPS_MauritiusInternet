...
public interface TransMoniRepositry extends JpaRepository<TransactionMonitoring, String> {
	...
	@Query(value = "select cim_account,cim_account_name,ipsx_account,ipsx_account_name,tran_date,value_date,sequence_unique_id,tran_audit_number,msg_type,tran_amount,entry_user,instr_id,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=cdtr_agt)cdtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=dbtr_agt)dbtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=instg_agt)instg_agt,tran_currency,tran_status,cbs_status,cbs_status_error,response_status,ipsx_status_error,tran_type_code from BIPS_TRANSACTION_MONITORING_TABLE where msg_type='INCOMING' and  trunc(tran_date)=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
	List<Object[]> findAllCustomIncome1(String tranDate);
	
	@Query(value = "select cim_account,cim_account_name,ipsx_account,ipsx_account_name,tran_date,value_date,sequence_unique_id,tran_audit_number,msg_type,tran_amount,entry_user,instr_id,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=cdtr_agt)cdtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=dbtr_agt)dbtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=instg_agt)instg_agt,tran_currency,tran_status,cbs_status,cbs_status_error,response_status,ipsx_status_error,tran_type_code from BIPS_TRANSACTION_MONITORING_TABLE where msg_type='INCOMING' and  trunc(tran_date)=?1 and tran_date is not null and tran_type_code in ('300','305') order by tran_date asc ", nativeQuery = true)
	List<Object[]> findAllCustomMERIncome1(String tranDate);

	@Query(value = "select cim_account,cim_account_name,ipsx_account,ipsx_account_name,tran_date,value_date,sequence_unique_id,tran_audit_number,msg_type,tran_amount,entry_user,instr_id,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=cdtr_agt)cdtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=dbtr_agt)dbtr_agt,(select a.bank_name from bips_other_bank_agent_table a where a.bank_agent=instg_agt)instg_agt,tran_currency,tran_status,cbs_status,cbs_status_error,response_status,ipsx_status_error,tran_type_code from BIPS_TRANSACTION_MONITORING_TABLE where msg_type='INCOMING' and  CIM_Account=?1 and tran_date is not null order by tran_date asc ", nativeQuery = true)
	List<Object[]> findAllCustomIncome1Merchant(String tranDate);
	...
	@Query(value = "select CIM_ACCOUNT,CIM_ACCOUNT_NAME,IPSX_ACCOUNT,IPSX_ACCOUNT_NAME,TRAN_DATE,VALUE_DATE,SEQUENCE_UNIQUE_ID,TRAN_AUDIT_NUMBER,MSG_TYPE,TRAN_AMOUNT,TRAN_CURRENCY,INIT_CHANNEL_ID,TRAN_STATUS,CBS_STATUS,CBS_STATUS_ERROR,RESPONSE_STATUS,IPSX_STATUS_ERROR from bips_outward_transaction_monitoring_table where cbs_status='CBS_DEBIT_OK' and tran_status='FAILURE' and trunc(tran_date)=?1 union all select  CIM_ACCOUNT,CIM_ACCOUNT_NAME,IPSX_ACCOUNT,IPSX_ACCOUNT_NAME,TRAN_DATE,VALUE_DATE,SEQUENCE_UNIQUE_ID,TRAN_AUDIT_NUMBER,MSG_TYPE,TRAN_AMOUNT,TRAN_CURRENCY,ENTRY_USER,TRAN_STATUS,CBS_STATUS,CBS_STATUS_ERROR,RESPONSE_STATUS,IPSX_STATUS_ERROR from bips_transaction_monitoring_table where cbs_status='CBS_CREDIT_OK' and tran_status='FAILURE'  and trunc(tran_date)=?1", nativeQuery = true)
...
//	List<Object[]> outwardSettlList(String tranDate);
	...
	@Query(value = "SELECT COUNT(CASE WHEN msg_type = 'INCOMING' AND TRAN_STATUS ='SUCCESS' THEN 0 END) as incS,\n" + 
			"COUNT(CASE WHEN msg_type = 'INCOMING' AND TRAN_STATUS ='FAILURE' THEN 0 END) as incF,\n" + 
			"COUNT(CASE WHEN msg_type = 'INCOMING' AND TRAN_STATUS ='IN_PROGRESS' THEN 0 END) as incIP,\n" + 
			"COUNT(CASE WHEN msg_type = 'INCOMING' AND TRAN_STATUS ='REVERSE_FAILURE' THEN 0 END) as incRF,\n" + 
			"COUNT(CASE WHEN msg_type = 'RTP' AND TRAN_STATUS ='SUCCESS' THEN 0 END) as RTPS,\n" + 
			"COUNT(CASE WHEN msg_type = 'RTP' AND TRAN_STATUS ='FAILURE' THEN 0 END) as RTPF,\n" + 
			"COUNT(CASE WHEN msg_type = 'RTP' AND TRAN_STATUS ='IN_PROGRESS' THEN 0 END) as RTPIP,\n" + 
			"COUNT(CASE WHEN msg_type = 'RTP' AND TRAN_STATUS ='REVERSE_FAILURE' THEN 0 END) as RTPRF,\n" + 
			"COUNT(CASE WHEN msg_type = 'INCOMING' THEN 0 END) as INCT,\n" + 
			"COUNT(CASE WHEN msg_type = 'RTP' THEN 0 END) as RTPT\n" + 
			"FROM BIPS_TRANSACTION_MONITORING_TABLE", nativeQuery = true)
	List<Object[]> findDashboardData();
	

	///Merchant Inward
	
		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListTotTxs(String tranDate);
		
		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and tran_status='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCLisSucTxs(String tranDate);

		@Query(value = "select nvl(count(sequence_unique_id),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and tran_status!='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListFailTxs(String tranDate);
		
		@Query(value = "select nvl(sum(tran_amount),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListTotAmt(String tranDate);
		
		@Query(value = "select nvl(sum(tran_amount),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and tran_status='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCLisSucAmt(String tranDate);

		@Query(value = "select nvl(sum(tran_amount),0) from bips_transaction_monitoring_table where trunc(tran_date)=?1 and tran_status!='SUCCESS' and (msg_type='INCOMING') and tran_type_code in ('300','305')", nativeQuery = true)
		String MerchantINCListFailAmt(String tranDate);
